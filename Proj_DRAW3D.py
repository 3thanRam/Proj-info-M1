...
def CHECKBOUNDS(LISTS,tindList):
    """
    Checks if all elements in the input lists are within the bounds of [0, L] and if boundaries are periodic doesn't interpolate between points of opposing sides
    ...
    """
    for ti,tind in enumerate(tindList):
        for indX,Xpos in enumerate(LISTS):
            if Xpos[tind]>L[indX] or Xpos[tind]<Linf[indX] or abs(Xpos[tind]-LISTS[indX][tindList[ti-1]])>0.7*(L[indX]-Linf[indX]):
                return(False)
    return(True) 
...
def GEN_CUBOID(L_up,L_down):
    ...
    R0 = [np.linspace(-radius+L_down[i],L_up[i]+radius,10**4) for i in range(3)]
    ...
    for i in range(2):
        for ii in range(2):
            X,Y,Z=np.concatenate((R0[0],RB1[i],RC1[i])),np.concatenate((RA1[i],R0[1],RC2[ii])),np.concatenate((RA2[ii],RB2[ii],R0[2]))
            PTLIST.append([X,Y,Z])
    return(PTLIST)
...
def DRAW3D(n1,n2,L0,dt,BOUNDARY_COND,DIM_Numb,ALL_TIME,TRACKING,COLPTS,Density,File_path_name):
                    ...
                    try:
                        if t_ai==0:
                            if len(PART[p][tr])>t_ai+2:
                                if CHECKBOUNDS(PART[p][tr],[t_ai,t_ai+1])==True:#CHECKBOUNDS([PART[p][tr][t_ai],PART[p][tr][t_ai+1]])==True:
                            ...
                            if len(PART[p][tr][0])-1>t_ai:
                                if t_ai>0:
                                    if CHECKBOUNDS(PART[p][tr],[t_ai-1,t_ai,t_ai+1])==True:#CHECKBOUNDS([PART[p][tr][t_ai-1],PART[p][tr][t_ai],PART[p][tr][t_ai+1]])==True:
                                        interAv=[(PART[p][tr][ind][t_ai-1]+PART[p][tr][ind][t_ai]+PART[p][tr][ind][t_ai+1])/3 for ind in range(len(PART[p][tr]))]
                                    else:
                                        interAv=[PART[p][tr][ind][t_ai] for ind in range(len(PART[p][tr]))]
                                else:
                                    if CHECKBOUNDS(PART[p][tr],[t_ai,t_ai+1])==True:#if CHECKBOUNDS([PART[p][tr][t_ai],PART[p][tr][t_ai+1]])==True:
                                        interAv=[(PART[p][tr][ind][t_ai]+PART[p][tr][ind][t_ai+1])/2 for ind in range(len(PART[p][tr]))]
                                    else:
                                        interAv=[PART[p][tr][ind][t_ai] for ind in range(len(PART[p][tr]))]
                            elif t_ai>0:
                                if CHECKBOUNDS(PART[p][tr],[t_ai-1,t_ai])==True:#if CHECKBOUNDS([PART[p][tr][t_ai],PART[p][tr][t_ai-1]])==True:
    ...
    CUBEDATA=GEN_CUBOID(L,Linf)
    for Xc,Yc,Zc in CUBEDATA:
        mlab.plot3d(Xc,Yc,Zc,line_width=0.01,color=(0,0,0),tube_radius=0.1)


    VOL=(L[0]-Linf[0])*(L[1]-Linf[1])*(L[2]-Linf[2])
    DENSITY[0]=[int(Dens0*VOL) for Dens0 in DENSITY[0]]
    DENSITY[1]=[int(Dens1*VOL) for Dens1 in DENSITY[1]]
    ...
    def update_scene(i):
        ...
        mlab.view(azimuth=theta,elevation=theta/2,roll=theta,distance=3.5*max(L),focalpoint=((L[0]+Linf[0])/2,(L[1]+Linf[1])/2,(L[2]+Linf[2])/2))
