...
def Denfct(DIM_Numb0,Dens,Trange):
    ...
    plt.loglog(Trange,Dens[0],color='red',label='Particle Density')
    plt.loglog(Ttheo,Dpart_theo,color='purple',label='Particle Theo')
    plt.loglog(Ttheo[1:],DLine,color='black',label=' t^-1')
    plt.loglog(Ttheo,D_theo0,color='grey',label='Particle FIT')

    if Dens[0]!=Dens[1]: 
        plt.loglog(Trange,Dens[1],color='blue',label='Anti-Particle Density')
        Dantipart_theo=d1/(1+d1*k2*Ttheo)
        plt.loglog(Ttheo,Dantipart_theo,color='green',label='Antiparticle Theo')
        D_theo1=[d1]+[d1**(1/2)*p_var*(D*dt)**(-DIM_Numb/4) for dt in Ttheo[1:]]
        plt.loglog(Ttheo,D_theo1,color='grey',label='AntiParticle FIT')
